...
(function (com) {
    (function (prezi) {
        (function (langdetect) {
            ...
            function initialize() {
                langdetect.CJK_MAP = {};
                for (var j in CJK_CLASSES) {
                    var cjk_list = CJK_CLASSES[j];
                    var representative = cjk_list.charAt(0);
                    for (var i = 0; i < cjk_list.length; ++i) {
                        langdetect.CJK_MAP[cjk_list.charAt(i)] = representative;
                    }
                }
            }
            initialize();
        })(prezi.langdetect || (prezi.langdetect = {}));
        var langdetect = prezi.langdetect;
    })(com.prezi || (com.prezi = {}));
    var prezi = com.prezi;
})(com || (com = {}));
    ...
    (function (prezi) {
        (function (langdetect) {
            ...
            var Detector = (function () {
                ...
                Detector.prototype.appendString = function (str) {
                ...
                Detector.prototype.cleaningText = function () {
                    var latinCount = 0;
                    var nonLatinCount = 0;
                    for (var i = 0; i < this.text.length; ++i) {
                        ...
                        if (c <= 'z' && c >= 'A') {
                            ++latinCount;
                        } else if (c >= '\u0300' && getUnicodeBlock(c) != 4 /* LATIN_EXTENDED_ADDITIONAL */) {
                            ++nonLatinCount;
                        }
                    }
                ...
                Detector.prototype.getProbabilities = function () {
                    var probabilities = this.detectBlock();
                    return this.sortProbability(probabilities);
                };

                Detector.prototype.sortProbability = function (prob) {
                    var list = [];
                ...
                Detector.prototype.detectBlock = function () {
                    this.cleaningText();
                    var ngrams = this.extractNGrams(this.text);
                    if (ngrams.length == 0) {
                ...
                Detector.prototype.initProbability = function () {
                ...
                Detector.prototype.extractNGrams = function (text) {
                    var list = [];
                    var ngram = new NGram();
                ...
                Detector.prototype.updateLangProb = function (prob, word, alpha) {
                ...
                Detector.normalizeProb = function (prob) {
                    var maxp = 0, sump = 0;
                    for (var i = 0; i < prob.length; ++i) {
                        sump += prob[i];
                    }
            ...
            var LangProbability = (function () {
                function LangProbability(lang, prob) {
                    this.lang = lang;
                    this.prob = prob;
                }
                LangProbability.prototype.toString = function () {
                    return '<' + this.lang + ': ' + this.prob + '>';
                };
                return LangProbability;
            })();
            langdetect.LangProbability = LangProbability;

            var LanguageProfiles = (function () {
                function LanguageProfiles() {
                    this.langList = [];
                    this.wordLangProbMap = {};
                }
                LanguageProfiles.loadFromJsonStrings = function (jsons) {
                ...
                LanguageProfiles.prototype.addJsonProfile = function (jsonString, index, numProfiles) {
            ...
            langdetect.LanguageProfiles = LanguageProfiles;
            ...
            var NGram = (function () {
                ...
                NGram.prototype.addChar = function (ch) {
                ...
                NGram.prototype.get = function (n) {
                ...
                NGram.normalize = function (ch) {
                    var cjk_map = com.prezi.langdetect.CJK_MAP;
                    var block = getUnicodeBlock(ch);
                    if (block == 0 /* BASIC_LATIN */) {
                        if (ch < 'A' || (ch < 'a' && ch > 'Z') || ch > 'z')
                            ch = ' ';
                    } else if (block == 1 /* LATIN_1_SUPPLEMENT */) {
                        if (NGram.LATIN1_EXCLUDED.indexOf(ch) >= 0)
                            ch = ' ';
                    } else if (block == 2 /* LATIN_EXTENDED_B */) {
                        // normalization for Romanian
                        if (ch == '\u0219')
                            ch = '\u015f'; // Small S with comma below => with cedilla
                        if (ch == '\u021b')
                            ch = '\u0163'; // Small T with comma below => with cedilla
                    } else if (block == 5 /* GENERAL_PUNCTUATION */) {
                        ch = ' ';
                    } else if (block == 3 /* ARABIC */) {
                        if (ch == '\u06cc')
                            ch = '\u064a'; // Farsi yeh => Arabic yeh
                    } else if (block == 4 /* LATIN_EXTENDED_ADDITIONAL */) {
                        if (ch >= '\u1ea0')
                            ch = '\u1ec3';
                    } else if (block == 6 /* HIRAGANA */) {
                        ch = '\u3042';
                    } else if (block == 7 /* KATAKANA */) {
                        ch = '\u30a2';
                    } else if (block == 8 /* BOPOMOFO */ || block == 9 /* BOPOMOFO_EXTENDED */) {
                        ch = '\u3105';
                    } else if (block == 10 /* CJK_UNIFIED_IDEOGRAPHS */) {
                        if (ch in cjk_map)
                            ch = cjk_map[ch];
                    } else if (block == 11 /* HANGUL_SYLLABLES */) {
                        ch = '\uac00';
                    }
                    return ch;
                };
                NGram.N_GRAM = 3;
                NGram.LATIN1_EXCLUDED = "\u00A0\u00AB\u00B0\u00BB";
                return NGram;
            })();

            var UnicodeBlock;
            (function (UnicodeBlock) {
                UnicodeBlock[UnicodeBlock["BASIC_LATIN"] = 0] = "BASIC_LATIN";
                UnicodeBlock[UnicodeBlock["LATIN_1_SUPPLEMENT"] = 1] = "LATIN_1_SUPPLEMENT";
                UnicodeBlock[UnicodeBlock["LATIN_EXTENDED_B"] = 2] = "LATIN_EXTENDED_B";
                UnicodeBlock[UnicodeBlock["ARABIC"] = 3] = "ARABIC";
                UnicodeBlock[UnicodeBlock["LATIN_EXTENDED_ADDITIONAL"] = 4] = "LATIN_EXTENDED_ADDITIONAL";
                UnicodeBlock[UnicodeBlock["GENERAL_PUNCTUATION"] = 5] = "GENERAL_PUNCTUATION";
                UnicodeBlock[UnicodeBlock["HIRAGANA"] = 6] = "HIRAGANA";
                UnicodeBlock[UnicodeBlock["KATAKANA"] = 7] = "KATAKANA";
                UnicodeBlock[UnicodeBlock["BOPOMOFO"] = 8] = "BOPOMOFO";
                UnicodeBlock[UnicodeBlock["BOPOMOFO_EXTENDED"] = 9] = "BOPOMOFO_EXTENDED";
                UnicodeBlock[UnicodeBlock["CJK_UNIFIED_IDEOGRAPHS"] = 10] = "CJK_UNIFIED_IDEOGRAPHS";
                UnicodeBlock[UnicodeBlock["HANGUL_SYLLABLES"] = 11] = "HANGUL_SYLLABLES";
            })(UnicodeBlock || (UnicodeBlock = {}));

            function getUnicodeBlock(c) {
                if (c.length != 1) {
                    throw Error("Cannot get unicode block for multiple characters");
                }
                var code = c.charCodeAt(0);
                if (0x0000 <= code && code <= 0x007F)
                    return 0 /* BASIC_LATIN */;
                if (0x0080 <= code && code <= 0x00FF)
                    return 1 /* LATIN_1_SUPPLEMENT */;
                if (0x0180 <= code && code <= 0x024F)
                    return 2 /* LATIN_EXTENDED_B */;
                if (0x0600 <= code && code <= 0x06FF)
                    return 3 /* ARABIC */;
                if (0x1E00 <= code && code <= 0x1EFF)
                    return 4 /* LATIN_EXTENDED_ADDITIONAL */;
                if (0x2000 <= code && code <= 0x206F)
                    return 5 /* GENERAL_PUNCTUATION */;
                if (0x3040 <= code && code <= 0x309F)
                    return 6 /* HIRAGANA */;
                if (0x30A0 <= code && code <= 0x30FF)
                    return 7 /* KATAKANA */;
                if (0x3100 <= code && code <= 0x312F)
                    return 8 /* BOPOMOFO */;
                if (0x31A0 <= code && code <= 0x31BF)
                    return 9 /* BOPOMOFO_EXTENDED */;
                if (0x4E00 <= code && code <= 0x9FFF)
                    return 10 /* CJK_UNIFIED_IDEOGRAPHS */;
                if (0xAC00 <= code && code <= 0xD7AF)
                    return 11 /* HANGUL_SYLLABLES */;
                return -1;
            }
...
(function (WebInterface) {
    function main() {
        document.body.innerHTML = '<h1 style="margin-bottom: 0;">Language Detector</h1>' + '<div><a href="https://github.com/laszlopandy/ts-language-detection">https://github.com/laszlopandy/ts-language-detection</a></div>' + '<div><a href="?languages=af,ar,bg,bn,cs,da,de,el,en,es,et,fa,fi,fr,gu,he,hi,hr,hu,id,it,ja,kn,ko,lt,lv,mk,ml,mr,ne,nl,no,pa,pl,pt,ro,ru,sk,sl,so,sq,sv,sw,ta,te,th,tl,tr,uk,ur,vi,zh-cn,zh-tw">Enable all supported languages</a></div>';
